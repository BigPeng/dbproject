...
def findInTable(attr,compare,value):
    if compare == "=":
        ...
        print "Unsupport compare:",compare
        return None
...
##        quickSort(joinResult,0,len(joinResult)-1,i)
    ...
    return records
#一趟希尔排序
def shellInsert(data,dk,keyIndex):
    for i in range(dk,len(data)):
        if data[i - dk][keyIndex] > data[i][keyIndex]:
            temp = data[i]
            sentry = data[i][keyIndex]
            j = i - dk
            while j >= 0 and sentry < data[j][keyIndex]:
                data[j + dk] = data[j]
                j -= dk
            data[j + dk] = temp
#希尔排序
def shellSort(data,keyIndex):
    dlta = [1001,523,97,47,5,3,1]
    for dk in dlta:
        shellInsert(data,dk,keyIndex)  
def readRecordInOrder(attrMap,joinResult):
    records = None
    title = joinResult[-1]
    joinResult = str2num(joinResult)
    length = len(joinResult)
    for i in range(len(title)):
        ...
        if len(attrs) == 0:
            continue
        lables = getAttrOrder(attrs)
        sigleTable = list([joinResult[k][i],k] for k in range(length-1))
        shellSort(sigleTable,0)
        tableFile = open(os.path.join(DATABASE,title[i]+'.tbl'),'r')
        for j in range(length-1):
            tableFile.seek(sigleTable[j][0])#定位
            line = tableFile.readline().split(SPLITTAG)
            lineRecord = []
            for lable in lables:
                lineRecord.append(line[lable])
            lineRecord.append(sigleTable[j][1])
            sigleTable[j] = lineRecord
        tableFile.close()
        shellSort(sigleTable,-1)#以最后一列为标准还原顺序
        for k in range(length-1):
            sigleTable[k].pop()#删掉列名        
        sigleTable.append(attrs)
        if records == None:
            records = sigleTable
        else:
            for k in range(length):
                records[k] += sigleTable[k]
    return records
...
def printResult(records):
    ...
    if length > 20:
        print 
        print 'The result hava %s rows, here is the fisrt 20 rows:' % (length - 1)
        length = 20
    divbline = 0
    fmt = ''
...
#在临时表中找group by 属性的列标
def getGroupLable(groups,titles):
    lables = []
    for each in groups:
...
#在临时表中找需要聚集属性的列标            
def getAggreLable(select,titles):
    lables = []
    opt = []
    rename = []
    countStar = None
    for each in select:
        if each[0] == '*' and each[1] == 'COUNT':#count(*)
            if each[2] == None:
                countStar = "COUNT(*)"
            else:
                countStar = each[2]
            continue
        if each[1] != None:#有聚集函数
            opt.append(each[1])
            if each[2] == None:#添加重命名
                rename.append(each[1]+'('+each[0]+')')
            else:
                rename.append(each[2])
            for i in range(len(titles)):
                if each [0] == titles[i]:
                    lables.append(i)
                    break
    return lables,opt,rename,countStar

#进行聚集
def getAggre(aggreAttr,option):
    if option == 'COUNT':
        return len(aggreAttr)
    sum = 0.0
    for each in aggreAttr:
        sum += float(each)
    if option == 'SUM':
        return sum
    if option == 'AVG':
        return sum/len(aggreAttr)
def groupby(records,select,groups):
    if groups == None:
        return records
    print 'records',records
    groupLables = getGroupLable(groups,records[-1])
    aggreLables,opt,rename,countStar = getAggreLable(select,records[-1])
    groupDic = collections.defaultdict(defaultli)
    records.pop()#删除表头
    for record in records:
        groupAttrs = ''
        for i in groupLables:
            groupAttrs += str(record[i])+"-+"#将group by属性拼接 
        groupDic[groupAttrs].append(record)
    print 'groupDic',groupDic
    mergeResult = []
    for k,v in groupDic.items():
        aggreResult = []
        for i in aggreLables:
            aggreAttr = list(each[i] for each in v)
            aggreResult.append(getAggre(aggreAttr,opt[i]))
        groupAtts = k.split("-+")
        groupAtts.pop()
        groupAtts += aggreResult
        if countStar != None:
            groupAtts.append(len(v))
        mergeResult.append(groupAtts)
    title = groups + rename
    if countStar != None:
        title.append(countStar)
    mergeResult.append(title)#补上表头
    print 'mergeResult[-1]',mergeResult[-1]
...
def nomalSelect(select,conditions,groups,orders):
    ...
    #t1 = time.time()
    #records = readRecordInOrder(attrMap,joinResult)
    records = readJoinRecord(attrMap,joinResult)
    #records = groupby(records,select,groups)
    #print 'readRecord',time.time()-t1
    saveJoin(records)
    printResult(records)     
